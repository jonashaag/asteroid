...
def main(conf):
    if 0:
        ...
        model_path = 'exp/train_dprnn_sep_clean_8kmin_145ed90f/_ckpt_epoch_19.ckpt'
        model = DPRNNTasNet(**conf['train_conf']['filterbank'], **conf['train_conf']['masknet'])
        checkpoint = torch.load(model_path)
        model.load_state_dict({k.split('.', 1)[1]: v for k, v in checkpoint['state_dict'].items()})
    ...
    import ds
    test_set = ds.getds(conf['sample_rate'], None)
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(test_set))):
            ...
            mix, sources = tensors_to_device(
                [torch.from_numpy(t) for t in test_set[idx]],
                device=model_device
            )
            ...
            utt_metrics['mix_path'] = test_set.ds.items[idx]
