...
def main(conf):
    ...
    train_loader = DataLoader(train_set, shuffle=True,
                              batch_size=int((2.3 if 'FP16' in os.environ else 1) * conf['training']['batch_size']),
    ...
    gpus = [int(x) for x in os.environ['GPUS'].split(',')]
    trainer = pl.Trainer(max_epochs=conf['training']['epochs'],
                         checkpoint_callback=checkpoint,
                         early_stop_callback=early_stopping,
                         default_root_dir=exp_dir,
                         gpus=gpus,
                         **(dict(amp_level='O2', precision=16) if 'FP16' in os.environ else {}),
